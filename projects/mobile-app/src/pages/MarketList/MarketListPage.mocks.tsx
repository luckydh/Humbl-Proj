import createApolloMock from "../../generated/createApolloMock";
import { GetMarketListDocument, MarketPeriodInput } from "../../generated/graphql";

const bitcoinAsset = (price: number, percentChangeOverPeriod: number) => ({
  currency: "USD",
  name: "Bitcoin",
  code: "BTC",
  logoImage:
    "data:image/svg+xml;base64,",
  price,
  percentChangeOverPeriod,
  __typename: "AssetMetricType",
});
const ethereumAsset = (price: number, percentChangeOverPeriod: number) => ({
  currency: "USD",
  name: "Ethereum",
  code: "ETH",
  logoImage:
    "data:image/svg+xml;base64,",
  price,
  percentChangeOverPeriod,
  __typename: "AssetMetricType",
});

export const MarketListPageMocks = () => [
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.All, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(10000, 0.02), ethereumAsset(45, 0.01)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.Hour_24, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(2000, -0.02), ethereumAsset(60, 0.1)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.Month_1, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(50000, 4.55), ethereumAsset(100, 0.4)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.Year_1, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(123456, 0.33), ethereumAsset(78, -0.333)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.Week_1, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(751248, 0.55), ethereumAsset(98, 0)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
  createApolloMock(
    GetMarketListDocument,
    { period: MarketPeriodInput.Hour_1, limit: 30 },
    {
      data: {
        getMarketList: {
          assets: [bitcoinAsset(78563159, 1.22), ethereumAsset(100, 0.01)],
        },
      },
    },
    {
      addTypename: false,
    }
  ),
];
