...
import { useGetMatchingEngineCreditCardsQuery } from "generated/graphql";
...
const CreditCardOffers: React.FC = () => {
  ...
  return (
    <div className="pb-6">
      <div className="text-white text-base text-center mt-8 mb-6 mx-6">{t("pages.matching-engine.credit-cards")}</div>
      ...
      <ul>
        ...
        {!loading &&
          data?.matchingEngineCards?.map((card, index) => (
              <li key={index}>
                <OfferItem image={card.image} link={card.link} name={card.name} />
              </li>
            ))}
      </ul>
    </div>
