import React from "react";
import { Story, Meta } from "@storybook/react/types-6-0";
import { action } from "@storybook/addon-actions";

import { CoinInfoScreen, CoinInfoScreenProps } from "./CoinInfoScreen";
import { MockedProvider } from "@apollo/client/testing";
import { GetassethistoryDocument, GetassetmetricDocument, MarketPeriodInput } from "generated/graphql";
import createApolloMock from "generated/createApolloMock";
import { assetHistoryMock } from "./asssetHistoryMock";
import { GraphQLError } from "graphql";
import { RecoilRoot } from "recoil";

export default {
  title: "CryptoWallet/Screens/CoinInfo",
  component: CoinInfoScreen,
  args: {
    setGraphActive: action("setGraphActive"),
  },
  parameters: {
    design: {
      type: "figma",
      url: "https://www.figma.com/file/QPSFMyF0BndoFizvebnr9b/HUMBL-Pay-Mobile?node-id=2885%3A11841",
    },

    storyshots: false,
  },
} as Meta;

const mockedAssetHistory = createApolloMock(
  GetassethistoryDocument,
  { assetNames: "undefined", period: MarketPeriodInput.Hour_24 },
  {
    data: {
      getAssetHistory: assetHistoryMock,
    },
  },
  { addTypename: false }
);

const mockedAssetMetrics = createApolloMock(
  GetassetmetricDocument,
  { assetNames: "undefined" },
  {
    data: {
      getAssetMetrics: {
        name: "Bitcoin",
        logoImage:
          "data:image/svg+xml;base64,",
      },
    },
  },
  { addTypename: false }
);

const mockedAssetHistoryError = createApolloMock(
  GetassethistoryDocument,
  { assetNames: "undefined", period: MarketPeriodInput.Hour_24 },
  {
    errors: [new GraphQLError("error")],
  },
  { addTypename: false }
);

const Template: Story<CoinInfoScreenProps & { mocks: any }> = ({ mocks, ...args }) => (
  <RecoilRoot>
    <MockedProvider mocks={mocks ?? [mockedAssetHistory, mockedAssetMetrics]} addTypename={false}>
      <CoinInfoScreen {...args} />
    </MockedProvider>
  </RecoilRoot>
);

export const Primary = Template.bind({});

Primary.args = {
  title: "Bitcoin",
};

export const HasError = Template.bind({});
HasError.args = {
  mocks: [mockedAssetHistoryError, mockedAssetMetrics],
};
